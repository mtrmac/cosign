...
Use the following image references in the below.

```shell
$ IMAGE=dlorenc/demo
$ IMAGE_DIGEST=$IMAGE@sha256:87ef60f558bad79beea6425a3b28989f01dd417164150ab3baab98dcbf04def8
```

For signing, we require digests to prevent race conditions, so be sure to use `$IMAGE_DIGEST`.
...
$ cosign sign --key other.key $IMAGE_DIGEST
Enter password for private key:
Pushing signature to: index.docker.io/dlorenc/demo:sha256-87ef60f558bad79beea6425a3b28989f01dd417164150ab3baab98dcbf04def8.sig
```
...
```shell
$ cosign sign --key other.key -a foo=bar $IMAGE_DIGEST
Enter password for private key:
Pushing signature to: index.docker.io/dlorenc/demo:sha256-87ef60f558bad79beea6425a3b28989f01dd417164150ab3baab98dcbf04def8
```
...
```shell
$ cosign sign --key cosign.key --payload README.md $IMAGE_DIGEST
...
```shell
$ cosign sign --key key.pem --upload=false $IMAGE_DIGEST
...
```shell
$ cosign generate $IMAGE_DIGEST | openssl...
```
